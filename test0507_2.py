''' 전일종가대비 최고 상승중인 상위 10개 코인 중에서 거래. 현재가 기록 후의 값들을 비교해 매수-매도가 산정. 지정가 거래 '''
...
while True:
    ...
    for k in range(1):
        for coin, rate in dictrate.items():
            if rate == max(dictrate.values()):
                listbestcoin.append(coin)
                dictrate[coin] = 0
                # print(coin, rate)
    print("분석할 코인: " + str(listbestcoin))

    dictrecord1 = {}
    dictrecord2 = {}
    for i in listbestcoin:
        timeprice = get_current(i)
        dictrecord1[i] = [timeprice]
    for k in range(9):
        for i in listbestcoin:
            timeprice = get_current(i)
            dictrecord1[i].append(timeprice)
    # print(dictrecord1)
    for i in listbestcoin:
        timeprice = get_current(i)
        dictrecord2[i] = [timeprice]
    for k in range(9):
        for i in listbestcoin:
            timeprice = get_current(i)
            dictrecord2[i].append(timeprice)
    # print(dictrecord2)
    dictavg1 = {}        
    for i in listbestcoin:
        dictrecord1[i].remove(max(dictrecord1[i]))
        dictrecord1[i].remove(min(dictrecord1[i]))
        dictavg1[i] = sum(dictrecord1[i]) / len(dictrecord1[i])
    dictavg2 = {}        
    for i in listbestcoin:
        dictrecord2[i].remove(max(dictrecord2[i]))
        dictrecord2[i].remove(min(dictrecord2[i]))
        dictavg2[i] = sum(dictrecord2[i]) / len(dictrecord2[i])
    dictrate = {}
    for i in listbestcoin:
        dictrate[i] = float(dictavg2[i]) / float(dictavg1[i])
    for coin, rate in dictrate.items():
        if rate == max(dictrate.values()):
            bestcoin = coin
    ...
    while True:
        ...
        if targetprice <= timeprice:
            buyhopeprice = get_buyhopeprice(timeprice, targetprice * 1.0025)
    ...
    while True:
        ...
        ''' 매도목표가 설정 및 매도 주문'''
        warningprice = float(maxprice) * 0.999
        if warningprice >= timeprice:    
            sellhopeprice = get_sellhopeprice(timeprice, warningprice * 0.9975)
            ...
            sell_total = dictbalance['KRW'] - krw
